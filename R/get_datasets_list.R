...
get_datasets_list <- function(datatype = c('all', 'timeseries', 'microdata', 'geospatial', 'other')) {
  ...
  count_url <- paste0('https://newdatacatalog.worldbank.org/search-service/search_api/datasets?limit=1&fields=[nid,uuid]&filter[status]=1&filter[field_wbddh_data_type]=', dtype)
  ...
  for (i in 1:iterations) {
    temp_offset <- (i - 1) * 500
    temp_url <- paste0('https://newdatacatalog.worldbank.org/search-service/search_api/datasets?limit=500&fields=[nid,uuid,title,field_wbddh_data_type]&filter[status]=1&filter[field_wbddh_data_type]=', dtype, '&offset=', temp_offset)
