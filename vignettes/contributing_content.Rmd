...
One thing to note here is that fields that take a controlled vocabulary for input use a mapped id for the value. You can convert your controlled vocabulary into the mapped ids using `map_tids()`.
...
### Adding supporting files/links/resources
Once the metadata is submitted, you can add data the files/links/resources. Again, you can add a resource to the Data Catalog by creating a named list of the resource metadata, formatting it in JSON format using `create_json_body()`, and creating a resource node using `create_resource()`.
```{r message=FALSE, warning=FALSE, paged.print=FALSE, eval=FALSE}
...
After the resource is added to the Data Catalog, it needs to be mapped to the dataset using `attach_resources_to_dataset()`. Multiple resources can be attached to a dataset by passing a list of resource nids.
```{r message=FALSE, warning=FALSE, paged.print=FALSE, eval=FALSE}
