...
One thing to note here is that some fields take a mapped id as the input. These correspond to fields which only accept a controlled list of values as input. You can get a mapping from the controlled list of values to the corresponding ids using `get_lovs()`.
...
```
Note that you need to set the "workflow_status" to "published" when you update the node.
