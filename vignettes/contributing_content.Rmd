...
To get the tids corresponding to the topics of the dataset, you can look for `"field_topic"`:

```{r}
tid_mapping[tid_mapping$machine_name == "field_topic", c("machine_name", "list_value_name", "tid")]
```

Using this, you can map the field_topic `"Poverty"` to its internal id `376`. Similarly, you can map the values for other fields taking a controlled vocabulary.
