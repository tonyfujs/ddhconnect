...
test_that("single resource works", {
  ...
  expect_equal(create_json_attach(resource_nids = 94934, root_url = root_url), jsonlite::toJSON(json_template, auto_unbox = T, pretty = T))
})
...
test_that("multiple resources work", {
  ...
  expect_equal(create_json_attach(resource_nids = c(94934, 94935), root_url = root_url), jsonlite::toJSON(json_template, auto_unbox = T, pretty = T))
})
