...
test_that("single resource works", {
  ...
  json_template$field_resources$und$target_id <- "Greater than 3 MB file (125939)"
  expect_equal(create_json_attach(resource_nids = 125939), jsonlite::toJSON(json_template, auto_unbox = T))
})
...
test_that("multiple resources work", {
  ...
  expect_equal(create_json_attach(resource_nids = c(125939, 125938)), jsonlite::toJSON(json_template, auto_unbox = T))
})
