...
test_that("multiple filters works", {
  expect_equal(build_search_query(filters = c('field_wbddh_data_type'=294, 'status'=1)),
               "limit=200&fields=[,,]&filter[field_wbddh_data_type]=294&filter[status]=1")
})

test_that("string filter value works", {
  expect_equal(build_search_query(filters = c('field_wbddh_data_type'='Microdata')),
               "limit=200&fields=[,,]&filter[field_wbddh_data_type]=Microdata")
})

test_that("combination of fields, filters and limit works", {
  expect_equal(build_search_query(fields = c('nid', 'uuid', 'title', 'field_contact_email', 'field_wbddh_data_type', 'status'),
