...
test_that("title field update works", {
  ...
  body <- create_json_body(c("title"="Test Create JSON"), node_type = "resource")
  json_template <- list()
...
test_that("text field update works", {
  body <- create_json_body(c("body"="Test Body"), node_type = "dataset")
  json_template <- list()
...
test_that("tid field update works", {
  body <- create_json_body(c("field_topic"="Energy and Extractives"), node_type = "dataset")
  json_template <- list()
